...
import { IPromotionProduct } from '@/interface/response/promotion';

export default function EditPromotionPage() {
  const router = useRouter();
  const params = useParams();
  const promotionId = typeof params.id === 'string' ? params.id : '';
  const { data: promotionData, isLoading, isError } = usePromotionDetail(promotionId);
  const updatePromotion = useUpdatePromotion();

  const [promotion, setPromotion] = useState<IPromotionUpdate>({
    name: '',
    description: '',
    discountPercent: 0,
    products: [],
  ...
  useEffect(() => {
    if (promotionData?.data) {
      const products = promotionData.data.products.map((product: any) => 
      ...
      setPromotion({
        ...
        discountPercent: promotionData.data.discountPercent,
        products: products,
        startDate: promotionData.data.startDate.split('T')[0],
        endDate: promotionData.data.endDate.split('T')[0],
  ...
  const validateForm = (): boolean => {
    ...
    if (promotion.discountPercent !== undefined && promotion.discountPercent <= 0) {
      newErrors.discountPercent = 'Phần trăm giảm giá phải lớn hơn 0';
    }
    
    if (promotion.discountPercent !== undefined && promotion.discountPercent > 100) {
      ...
      await updatePromotion.mutateAsync(
        { promotionId, payload: promotion },
        {
          ...
          onError: (error) => {
            ...
            toast.error('Cập nhật chương trình khuyến mãi thất bại: ' + (error.message || 'Không xác định'));
          }
        }
      );
    } catch (error) {
  ...
  const getSelectedProductDetails = (productId: string) => {
    // First check in the original promotion data
    if (promotionData?.data?.products) {
      const productFromOriginal = promotionData.data.products.find(
        (product: any) => product._id === productId || product === productId
      );
      if (productFromOriginal && typeof productFromOriginal !== 'string') {
        return productFromOriginal as IPromotionProduct;
      }
    }
    
    // Then check in the products data from the API
    if (productsData?.data?.products) {
      return productsData.data.products.find(product => product._id === productId) as IProduct;
    }
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-start">
          <Breadcrumb>
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Chỉnh sửa chương trình khuyến mãi</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
          <Button
        ...
        <Card>
          <CardContent className="p-6 text-center">
            <p className="text-red-500 mb-4">Đã xảy ra lỗi khi tải thông tin chương trình khuyến mãi. Vui lòng thử lại sau.</p>
            <Button
              variant="outline"
              onClick={() => router.back()}
            >
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              ...
              <CardContent className="space-y-4 text-maintext">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select
                      value={promotion.status || 'HOAT_DONG'}
                ...
                {selectedProductIds.length === 0 ? (
                  ...
                  <div className="space-y-4">
                    {selectedProductIds.map(productId => {
                      ...
                      // Determine if the product has images
                      let productImage: string | null = null;
                      
                      // Handle IProduct type which has variants
                      if ('variants' in product && product.variants?.[0]?.images?.length) {
                        productImage = product.variants[0].images[0];
                      } 
                      // Handle IPromotionProduct type which has images directly
                      else if ('images' in product && product.images?.length) {
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                <CardTitle>Cập nhật chương trình khuyến mãi</CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  <div className="flex gap-2 items-center text-sm">
                    <span className="font-semibold min-w-20">Mã khuyến mãi:</span>
                    <span className="bg-gray-100 px-2 py-1 rounded-md">{promotionData?.data?.code}</span>
                  </div>
                  {promotionData?.data?.createdAt && (
                    <div className="flex gap-2 items-center text-sm">
                      <span className="font-semibold min-w-20">Ngày tạo:</span>
                      <span>
                        {new Date(promotionData.data.createdAt).toLocaleDateString('vi-VN', {
                          day: '2-digit',
                          month: '2-digit',
                          year: 'numeric',
                          hour: '2-digit',
                  ...
                  {promotionData?.data?.updatedAt && (
                    <div className="flex gap-2 items-center text-sm">
                      <span className="font-semibold min-w-20">Cập nhật:</span>
                      <span>
                        {new Date(promotionData.data.updatedAt).toLocaleDateString('vi-VN', {
                          day: '2-digit',
                          month: '2-digit',
                          year: 'numeric',
                          hour: '2-digit',
                ...
                <div className="mt-6">
                  <p className="text-sm text-gray-500 mb-6">
                    Vui lòng kiểm tra kỹ thông tin trước khi cập nhật chương trình khuyến mãi.
                  </p>
                  <Button 
                    type="submit" 
                    className="w-full"
