...
export const InteractiveHoverButton = ({ children, className }: { children: React.ReactNode, className?: string }) => {
    return (
        <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className={`!h-10 relative inline-flex items-center justify-center px-6 py-3 overflow-hidden font-medium text-white bg-primary rounded-sm group ${className ?? ''}`}
        >
